# Connect to panorama with pan.xapi module, grab all connected firewalls, go to each, parse config and test nat rules to obtain public IPs, compile a list in a PAN EDL format
...
PA_IP_list=list(findkeys(output,'public_ip_address'))

f=open("/var/nginx/html/iprange/PA_IP_list.txt", "w")
f.write("\n".join(list(set(PA_IP_list))))
